...
class QrisSection extends StatelessWidget {
  QrisSection({super.key, required this.controller});

  final TagihanPaymentBillController controller;

  final _ss = ScreenshotController();
    ...
    return Column(
      children: [
                ...
                Container(
                  ...
                  child: SizedBox(
                      ...
                      child: SfBarcodeGenerator(
                          value: controller.paymentCustomer?.qrCode?.code,
                          symbology: QRCode())),
                ),
                Gap(24.h),
                Row(
        ...
        AppButton.primary(
          ...
          onPressed: () async {
            try {
              final bytes = await _ss.capture(pixelRatio: 3.0);
              if (bytes == null) return;

              final name = 'qris_${DateTime.now().millisecondsSinceEpoch}';
              ...
              if (ok) {
                AppSnackbar.show(
                  message: 'Berhasil menyimpan ke Galeri',
